...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options['actor']:
            ...
            if actor is None: raise CommandError(f'Active actor not found: {options["actor"]}')
        financial = self.has_financial_rows(path, selected)
        if options['apply'] and financial and actor is None:
            raise CommandError('--actor <username> is required to apply order payments or purchase receiving.')

        plan = Plan()
        with transaction.atomic():
            self.run(path, selected, actor, plan)
            transaction.set_rollback(True)
        self.print_plan(plan)
        for warning in plan.warnings: self.stderr.write(f'WARNING: {warning}')
        if plan.errors:
            for error in plan.errors: self.stderr.write(error)
        ...
        try:
            with transaction.atomic():
                self.run(path, selected, actor, applied)
                if applied.errors: raise BlockingError('\n'.join(applied.errors))
        except (BlockingError, ValidationError, ValueError, PostingError) as exc:
    ...
    def print_plan(self, plan):
        ...
        for section in ORDER:
            if plan.counts[section]: self.stdout.write(f'{section}:\n  ' + '\n  '.join(f'{key}={value}' for key, value in sorted(plan.counts[section].items())))
        if plan.stock:
            self.stdout.write('stock_effect:')
            for code, quantity in sorted(plan.stock.items()): self.stdout.write(f'  {code} +{quantity.quantize(Decimal(".001"))} {plan.stock_units[code]}')
        if plan.payments:
            self.stdout.write('payments_effect:')
            for method, amount in sorted(plan.payments.items()): self.stdout.write(f'  {method} {amount}')
